...
function renderizarProductos(productos) {
    ...
    lista.innerHTML = productos.map(producto => `
        <div class="product-item" data-id="${producto.id}">
            <img src="${producto.imagen || 'default.png'}" width="99" height="99">
            <h3>${producto.nombre}</h3>
            <p>${producto.presentacion} - $${producto.precio}</p>
            <button onclick="seleccionarProducto(${producto.id})">Editar</button>
...
async function guardarProducto() {
    ...
    try {
        ...
        const url = productoSeleccionado ? `${API_URL}/id/${productoSeleccionado}` : API_URL;
        ...
        mostrarExito(productoSeleccionado ? "Producto actualizado" : "Producto guardado");
        limpiarCampos();
        cargarProductos();
    } catch (error) {
